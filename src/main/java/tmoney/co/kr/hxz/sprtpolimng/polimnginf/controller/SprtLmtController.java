 ...
 *
 * 관련 테이블:
 * - tbhxzd208  : HXZ_지원금한도관리 (금액/건수 한도 이력 관리)
 * - tbhxzm201  : HXZ_교통복지서비스관리 (서비스 정보)
 * - tbhxzm202  : HXZ_교통복지서비스유형관리 (서비스 유형 정보)
 *
 * 주요 기능:
 * - 지원 한도 목록(서비스/서비스유형 기준) 조회
 * - 금액(분기/월)·건수 한도를 통합한 3in1 모달 신규/수정
 * - 기존 한도 존재 여부 및 분기 중복 범위 조회 (프론트 confirm 로직용)
 */
@Controller
...
public class SprtLmtController {
     ...
     * 지원 한도 내역 조회 화면.
     *
     * 서비스/서비스유형 기준으로 tbhxzd208(지원금한도관리)에 등록된 최신 한도 정보를
     * 페이징 조회하여 목록 화면에 렌더링한다.
     *
     * 사용 VO:
     ...
     * 지원 한도 3in1 신규 모달 오픈.
     *
     * 특정 서비스/서비스유형에 아직 한도(금액/건수)가 한 번도 등록되지 않은 경우,
     ...
     * 지원 한도 3in1 수정 모달 오픈.
     *
     * 특정 서비스/서비스유형에 이미 등록된 한도(금액/건수)를 조회하여
     ...
     * 지원 한도(금액/건수) 저장 API.
     *
     * 3in1 모달에서 입력한 금액/건수 한도 정보를 서버로 전송하여 저장한다.
     * 한도 구분/타입에 따라 내부에서 아래와 같이 처리한다.
     *
     * - 금액(분기/월) 한도:
     *   동일 기간 내 기존 버전 use_yn='N' 처리 후 신규 버전 insert
     * - 건수 한도:
     *   동일 서비스/유형의 기존 건수 한도 전체 use_yn='N' 후 신규 버전 insert
     *
     * 실제 이력 관리 및 use_yn 업데이트 로직은
     * {@link SprtLmtService#insertSprtLmtAmt(InstReqVO)} 에서 처리한다.
     *
     ...
     *
     * 프론트에서 신규/수정 모달을 띄우기 전에 호출하여,
     * 해당 서비스/서비스유형에 기존 한도가 있는지, 있다면 어떤 형태인지(금액/건수, 월/분기)를 확인한다.
     *
     * 반환 VO {@link SprtLmtExistResVO} 내용 예시:
     * - exists       : 해당 서비스/유형에 활성(use_yn='Y') 한도가 하나라도 존재하는지 여부
     *                  (금액/건수 구분 없이 전체 기준)
     * - qtRanges     : 기존 분기 한도 범위 리스트 (분기 중복 여부 체크용)
     * - svcLmtDvsCd  : 서비스 단위 한도 구분 (01=금액, 02=건수) - 단일 유형인 경우 설정
     * - svcLmtTypCd  : 서비스 단위 한도 타입 (01=월, 02=분기/건수) - 단일 유형인 경우 설정
     * - multiKinds   : 한도 유형이 둘 이상인 경우 true (프론트에서 안내 문구/처리 분기용)
     *
     * 프론트 사용 예:
     * - 기존 한도 존재 시, 신규 등록 전에 "기존 한도를 종료하고 새로 등록할지" confirm 알림 표시
     * - 분기 한도인 경우, qtRanges를 참고하여 중복 기간 여부를 검사
     *
     * @param tpwSvcId    서비스 ID
     * @param tpwSvcTypId 서비스 유형 ID
